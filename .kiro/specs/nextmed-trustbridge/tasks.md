...
- [x] 3. 同意管理と評価機能を実装する
- [x] 3.1 (P) 同意条件の登録・更新を実装する
  - 対象データ種別、共有先、利用目的、有効期間を同意条件として保持する。
  - _Requirements: 2.1, 2.5_
- [x] 3.2 (P) 同意の部分撤回と版管理を実装する
  - 部分撤回対象のみ無効化し、履歴と最新状態を整合させる。
  - _Requirements: 2.3, 2.4, 2.5_
- [x] 3.3 同意評価ロジックを判定サービスへ統合する
  - 同意不一致時の拒否理由を判定結果へ反映する。
  - _Requirements: 2.2, 2.3, 2.5_

- [x] 4. 連携要求判定フローを実装する
- [x] 4.1 連携要求受付と追跡ID発行を実装する
  - 要求の妥当性検証を行い、全処理を追跡IDで関連付ける。
  - _Requirements: 3.1, 3.5_
- [x] 4.2 参加者信頼・同意条件を統合した判定を実装する
  - 条件一致時の承認、条件不一致時の不承認を理由付きで返す。
  - 判定不能時は安全側（保留/不承認）へ遷移する。
  - _Requirements: 3.2, 3.3, 5.2_
- [x] 4.3 判定状態参照と通知連携を実装する
  - 処理中/完了ステータスの参照を可能にする。
  - 承認結果の通知連携ポートを実装し、通知失敗時の扱いを定義する。
  - _Requirements: 3.2, 3.4, 3.5_

- [x] 5. 監査証跡と外部監査連携を実装する
- [x] 5.1 監査イベントの追記専用記録と検索を実装する
  - 操作主体、時刻、対象、結果を検索可能な形で保持する。
  - _Requirements: 4.1, 4.3, 4.5_
- [x] 5.2 監査整合チェックと改変疑い検知を実装する
  - 欠落・改変疑いを検知してアラート通知する。
  - _Requirements: 4.2, 4.5_
- [x] 5.3 監査エクスポートジョブを実装する
  - エクスポート認可条件、期間上限、実行履歴追跡を適用する。
  - _Requirements: 4.4, 4.5_

- [x] 6. 判定・監査整合の回復戦略を実装する
- [x] 6.1 Outbox駆動の非同期連携フローを実装する
  - 判定結果からアンカー記録と監査記録を順序実行する。
  - _Requirements: 3.5, 4.5_
- [x] 6.2 片側成功時の補償フローを実装する
  - アンカー成功/監査失敗、アンカー失敗/監査成功の補償処理を実装する。
  - _Requirements: 3.3, 4.5, 5.5_
- [x] 6.3 依存障害時の再試行と保留再評価を実装する
  - 再試行上限、保留維持、再評価完了条件を状態機械で管理する。
  - _Requirements: 5.1, 5.2, 5.3, 5.5_

- [x] 7. 運用可観測性と管理UIを実装する
- [x] 7.1 (P) 運用メトリクス収集を実装する
  - 成功率、遅延、エラー件数を集計し、運用者が参照できるようにする。
  - _Requirements: 5.4_
- [x] 7.2 常駐API連携の管理UIフローを実装する
  - Next.js App Router の画面境界を維持しつつ、参加者管理、同意管理、要求状態、監査検索の主要操作を統合する。
  - API 呼び出し責務（Server Action / Route Handler / Client fetch）を明確化し、同一処理の重複実装を回避する。
  - _Requirements: 1.1, 2.1, 3.4, 4.3, 5.3_
- [x] 7.3 障害時の可視化と運用ハンドリングを実装する
  - 保留件数、再評価待ち、依存障害状態を運用画面で把握できるようにする。
  - _Requirements: 5.1, 5.3, 5.4_

- [x] 8. テストと品質ゲートを整備する
- [x] 8.1 ドメイン単体テストを整備する
