...
const knowledgeBase: Record<string, { q: RegExp; a: string }[]> = {
  肺結節: [
    { q: /鑑別|考えられる|診断/i, a: "肺結節の鑑別診断としては、(1) 原発性肺癌（腺癌が最も多い）、(2) 転移性肺腫瘍、(3) 良性肉芽腫（結核、サルコイドーシス）、(4) 過誤腫が挙げられます。12mmかつ辺縁不整という所見からは、悪性の可能性を考慮すべきです。" },
    { q: /治療|方針|プラン/i, a: "Lung-RADS 4Bの場合、推奨される方針は：(1) 造影胸部CT精査、(2) PET-CT検査、(3) 経皮的針生検またはEBUS-TBNA、(4) 呼吸器外科カンファレンスでの検討です。3ヶ月以内のフォローCTも考慮してください。" },
    { q: /ガイドライン|基準|エビデンス/i, a: "肺癌取扱い規約第8版およびACR Lung-RADS v2022に準拠しています。10mm以上の充実性結節でLung-RADS 4B相当。Fleischner Societyガイドラインでは、PET-CTまたは生検が推奨されます。" },
  ],
  糖尿病性腎症: [
    { q: /鑑別|考えられる|診断/i, a: "糖尿病性腎症の確認には、(1) 尿中アルブミン/Cr比の推移確認、(2) eGFR推移の評価、(3) 網膜症の有無（合併することが多い）、(4) 腎生検の検討が重要です。IgA腎症等の合併も除外が必要です。" },
    { q: /治療|方針|プラン/i, a: "CKD Stage 3bに対する治療方針：(1) SGLT2阻害薬（ダパグリフロジン）の導入、(2) RAS阻害薬の最適化、(3) 血糖管理（HbA1c 7.0%以下目標）、(4) 食事療法（蛋白制限0.8g/kg/日）を推奨します。" },
  ],
  心房細動: [
    { q: /鑑別|考えられる|診断/i, a: "持続性心房細動の精査として、(1) 甲状腺機能検査（TSH）、(2) 心エコー（弁膜症の除外）、(3) BNP/NT-proBNP測定、(4) CHA2DS2-VAScスコア評価が必要です。本症例はスコア4点で高リスクです。" },
    { q: /治療|方針|プラン/i, a: "CHA2DS2-VASc 4点の管理：(1) DOAC（エドキサバン60mg/日）開始、(2) レートコントロール（ビソプロロール2.5mg）、(3) カテーテルアブレーションの適応検討、(4) 心不全合併のモニタリングを推奨します。" },
  ],
  default: [
    { q: /鑑別|考えられる|診断/i, a: "現在の検査所見からは、主病名に加えて合併症の有無を確認することを推奨します。追加の血液検査・画像検査の結果を統合的に評価してください。" },
    { q: /治療|方針|プラン/i, a: "エビデンスに基づいた標準治療を基本としつつ、患者の年齢・併存疾患・意向を考慮した個別化医療の方針を検討してください。" },
  ],
}
    ...
    <div className="flex flex-col rounded-2xl border border-border bg-card overflow-hidden">
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-medium text-foreground">AI診断アシスタント</span>
          {patient && (
        ...
        <div className="flex items-center gap-2">
          {messages.length > 0 && (
            <span className="text-[10px] text-muted-foreground">{messages.length}件</span>
          )}
          {isExpanded ? (
      ...
      <AnimatePresence>
        {isExpanded && (
          ...
          >
            {/* Messages area */}
            <div ref={scrollRef} className="flex flex-col gap-3 overflow-y-auto p-4" style={{ maxHeight: 280, minHeight: 160 }}>
            ...
            <form onSubmit={handleSubmit} className="flex items-center gap-2 border-t border-border p-3">
              <input
                ...
                placeholder={patient ? "質問を入力..." : "患者を選択してください"}
