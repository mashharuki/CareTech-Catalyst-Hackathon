...
function validateRegister(body: RegisterRequestBody): ValidationIssue[] {
  ...
  if (body.trustLevel && !['low', 'medium', 'high'].includes(body.trustLevel)) {
    issues.push({ field: 'trustLevel', message: 'trustLevelはlow|medium|highのいずれか' })
  }
  return issues
}
...
export function buildParticipantsRouter(): Hono {
  ...
  router.post('/register', async (c) => {
    ...
    if (participantsById.has(body.id)) {
      return c.json({ error: 'DUPLICATE', issues: [{ field: 'id', message: '既に登録されています' }] }, 409)
    }
    const existingId = emailIndex.get(body.email.toLowerCase())
    if (existingId) {
      return c.json({ error: 'DUPLICATE', issues: [{ field: 'email', message: 'メールは既に使用されています' }] }, 409)
    }
  ...
  router.post('/:id/state', async (c) => {
    ...
    if (body.action) {
      ...
      if (body.action === 'suspend') {
        if (p.status !== 'suspended') {
          p.status = 'suspended'
          p.history.push({ timestampMs: nowMs, actorRole: actor, action: 'suspend', fromStatus: prev, toStatus: p.status, reason: body.reason })
          changed = true
        }
      } else if (body.action === 'resume' || body.action === 'activate') {
        if (p.status !== 'active') {
          p.status = 'active'
          p.history.push({ timestampMs: nowMs, actorRole: actor, action: body.action, fromStatus: prev, toStatus: p.status, reason: body.reason })
          changed = true
        }
      } else {
        return c.json({ error: 'VALIDATION_FAILED', issues: [{ field: 'action', message: 'actionはactivate|suspend|resume' }] }, 400)
      }
    }
    ...
    if (body.trustLevel) {
      if (!['low', 'medium', 'high'].includes(body.trustLevel)) {
        return c.json({ error: 'VALIDATION_FAILED', issues: [{ field: 'trustLevel', message: 'trustLevelはlow|medium|high' }] }, 400)
      }
      const prevTrust: TrustLevel = p.trustLevel
      if (prevTrust !== body.trustLevel) {
        p.trustLevel = body.trustLevel
        p.history.push({ timestampMs: nowMs, actorRole: actor, action: 'trust-update', fromTrust: prevTrust, toTrust: p.trustLevel, reason: body.reason })
        changed = true
      }
    }
