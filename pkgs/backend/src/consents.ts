...
function validateRegister(body: RegisterConsentBody): ValidationIssue[] {
  ...
  if (!notEmptyStringArray(body.dataTypes))
    issues.push({ field: "dataTypes", message: "dataTypesは非空の文字列配列" });
  if (!notEmptyStringArray(body.recipients))
    issues.push({
      field: "recipients",
      message: "recipientsは非空の文字列配列",
    });
  if (!notEmptyStringArray(body.purposes))
    issues.push({ field: "purposes", message: "purposesは非空の文字列配列" });
  if (
    typeof body.validFromMs !== "number" ||
    typeof body.validToMs !== "number"
  )
    issues.push({
      field: "validFromMs/validToMs",
      message: "有効期間は数値ミリ秒",
    });
  else if (body.validFromMs > body.validToMs)
    issues.push({
      field: "validity",
      message: "validFromMs <= validToMs が必要",
    });
  return issues;
}
...
export function buildConsentsRouter(): Hono {
  ...
  router.post("/register", async (c) => {
    ...
    if (consentsById.has(body.id))
      return c.json(
        {
          error: "DUPLICATE",
          issues: [{ field: "id", message: "既に存在します" }],
        },
        409,
      );
